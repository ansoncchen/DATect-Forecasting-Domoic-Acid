...
**IMPORTANT - Testing is SLOW**: Each evaluation run takes **>10 minutes**. Plan accordingly:
...
## Priority 2: Modernize Data Pipeline

### Current State (Problems)
- Heavy reliance on NetCDF, Parquet, and Pandas
- Crude implementation, not production-ready
- Slow processing times for dataset regeneration (30-60 min)

### Goals
- Implement industry-standard ETL patterns
- Consider faster alternatives (Polars, DuckDB, Rust-based tools)
- Add proper data validation, logging, and error handling
- Maintain backward compatibility with existing interfaces
- Keep `dataset-creation.py` as the main entry point

### Constraints
- Preserve existing data formats for model compatibility
- Don't break the forecasting engine interface
- Don't break `run_datect.py` or `verify_temporal_integrity.py`
...
### Current State
- 3+ years of accumulated technical debt ("vibe coding")
...
### Do NOT
- Change core algorithmic logic without testing
- Remove "disabled" features entirely (keep for future experiments)
- Break existing API endpoints or CLI commands
...
## Build & Run Commands
See AGENT.md for detailed build and run instructions.

Quick reference:
```bash
python3 run_datect.py              # Run full system
python3 verify_temporal_integrity.py   # Verify no data leakage
python3 precompute_cache.py        # Evaluate model performance
