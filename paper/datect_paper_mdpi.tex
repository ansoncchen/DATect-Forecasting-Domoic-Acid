...
% ══════════════════════════════════════════════════════════════════════════════
\abstract{
Domoic acid (DA) contamination of shellfish is a recurring public health crisis along the U.S.\ Pacific Northwest coast, yet no quantitative forecasting system has been published for Washington and Oregon. We present DATect, a one-week-ahead forecasting system for 10 monitoring sites spanning \SI{600}{\kilo\meter} of coastline, trained on 21 years of satellite, climate, hydrological, and biological data (2003--2023). DATect uses a per-site XGBoost + Random Forest ensemble whose per-site outer configuration (model weights, prediction clipping) is selected by a constrained chronological cross-validation on pre-2022 data only, making the 2022--2023 temporal holdout fully unbiased. On that holdout, a deterministic evaluation using every real DA measurement in the window ($N = 404$) gives pooled $R^2 = 0.485$ [95\% CI: 0.330, 0.604], MAE $= \SI{6.76}{\microgrampeg}$, and spike recall $= 0.857$ at the operational \SI{20}{\microgrampeg} threshold. A complementary 5-seed bootstrap on the same window yields $R^2 = 0.433 \pm 0.092$, consistent with the deterministic estimate. Per-site holdout skill is positive at 9 of 10 sites under the deterministic protocol, with the five Washington sites and Clatsop Beach in the $R^2 = 0.49$--$0.63$ range. Because MSE-optimized regression conservatively under-predicts threshold crossings, we also train a dedicated transition-focused binary classifier; the resulting hybrid alert (classifier $\cup$ regression trigger at \SI{12}{\microgrampeg}) raises spike recall further while keeping false alerts manageable. DATect is deployed as a publicly accessible web dashboard. All tunable hyperparameters and the chronological evaluation windows are stored in a single source-of-truth JSON file (\texttt{config/tuned\_hyperparameters.json}) with per-key provenance, enabling fully reproducible re-tuning.
}
...
Hybrid alert (cls.\ + 12 trigger) & \textbf{0.734} & \textbf{0.859} & 0.344 & 0.661 \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
...
The near-tie between the tree-based models and Ridge regression further shows that most forecast signal comes from the feature set and the per-site design, not from an intrinsically superior model family. Removing per-site customization costs $\Delta R^2 = -0.158$ in the component ablation (Table~\ref{tab:ablation}; the perturbation study reports $-0.103$ for the same removal from a slightly earlier baseline---see Table~\ref{tab:perturbation} caption), far more than any other single component. Likewise, model-selection robustness in Appendix~\ref{app:weight-robustness} shows that choosing XGBoost versus Random Forest per site changes aggregate $R^2$ only modestly.
...
\midrule
Full DATect (baseline)    & 0.414 & ---      & 6.38 & Current config (dev set) \\
No per-site custom.       & 0.256 & $-0.158$ & 6.69 & Largest impact \\
No interp.\ training      & 0.385 & $-0.029$ & 6.61 & Data augmentation \\
No obs-order lags         & 0.413 & $-0.001$ & 6.39 & Minimal standalone \\
