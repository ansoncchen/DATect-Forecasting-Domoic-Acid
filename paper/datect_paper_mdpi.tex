...
\begin{table}[H]
\caption{Per-site ensemble performance on the 2022--2023 temporal holdout, the most conservative unbiased evaluation. Multi-seed bootstrap with 5 random-anchor seeds (42--46) using the leak-free grid-search per-site outer configuration (Section~\ref{sec:results-temporal-holdout}, Limitations item 3). Per-seed $N$ shown as mean across seeds; per-site $R^2$ shown as mean $\pm$ std and full single-seed range. Pooled holdout: $R^2 = 0.433 \pm 0.092$ (range 0.32--0.55), MAE $= 5.93 \pm 0.34$, $N \approx 160$ per seed.}
...
\midrule
Quinault      & 26 & \textbf{.582 $\pm$ .133} & [.434, .770]  & 3.04  & High \\
Long Beach    & 19 & \textbf{.549 $\pm$ .166} & [.303, .810]  & 3.88  & High \\
Copalis       & 29 & \textbf{.532 $\pm$ .141} & [.283, .680]  & 3.14  & High \\
Clatsop Bch   & 12 & \textbf{.532 $\pm$ .192} & [.311, .884]  & 5.79  & High \\
Twin Harbors  & 18 & \textbf{.525 $\pm$ .178} & [.360, .852]  & 4.07  & High \\
Kalaloch      &  9 & \textbf{.414 $\pm$ .171} & [.201, .674]  & 2.82  & Mod. \\
\addlinespace
Coos Bay      &  9 & \textbf{.673 $\pm$ .113} & [.515, .818]  & 12.29 & High$^\dagger$ \\
\addlinespace
Newport       & 20 & $-$0.36 $\pm$ 0.69 & [$-$1.70, .274]  & 15.71 & Low$^\ddagger$ \\
Gold Beach    & 16 & $-$2.63 $\pm$ 4.17 & [$-$10.9, $-$.082] & 5.92 & Low$^\ddagger$ \\
Cannon Bch    & --- & no spikes in holdout & --- & --- & N/A$^*$ \\
\midrule
\textbf{Overall} & \textbf{$\sim$160} & \textbf{.433 $\pm$ .092} & \textbf{[.318, .551]} & \textbf{5.93} & --- \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
...
The 2022--2023 temporal holdout (Table~\ref{tab:temporal-holdout}) provides the strictest unbiased validation: it is downstream of the leak-free grid-search selection window, and the 5-seed bootstrap directly quantifies how much single-seed point estimates can vary at this $N$. Pooled holdout $R^2 = 0.433 \pm 0.092$ (single-seed range 0.32--0.55 across seeds 42--46). The standard-eval value of 0.241 (Table~\ref{tab:model-comparison}, single seed 123 from the 40\% random-anchor sample) and the holdout 5-seed mean of 0.433 are not directly comparable as point estimates—they sample different test sets at different $N$—but the holdout multi-seed range (0.32--0.55) is the publishable measure of generalization skill.
