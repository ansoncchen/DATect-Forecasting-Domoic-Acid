...
\addlinespace
C-HARM \cite{Anderson2016} & California & Grid & Ongoing & Satellite+model & PN prob. & Nowcast & Yes \\
\addlinespace
...
\midrule
\textbf{Per-site sel.\ (XGB or RF)}$^{\dagger}$ & \textbf{0.215} [0.096, 0.342] & \textbf{6.42} [5.78, 7.15] & \textbf{17.79} & --- \\
...
\end{tabular}
\vspace{0.3em}\\
\footnotesize{Clip~$q$: predictions clipped to this quantile of training targets. ``---'' = global default ($q = 0.99$) or no hard ceiling. Clip max in \si{\microgrampeg}. All sites were assigned single-model weights ($w \in \{0,1\}$); blended weights were tested but did not improve over single-model selection (Appendix~\ref{app:weight-robustness}).}
\end{table}
