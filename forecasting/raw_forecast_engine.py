...
from .per_site_models import (
    ...
    get_site_quantile_alpha,
    get_site_use_pooled_training,
...
class RawForecastEngine:
    ...
    # ------------------------------------------------------------------
    # Pooled cross-site training data for regression (struggle sites)
    # ------------------------------------------------------------------

    def _build_pooled_regression_data(
        self,
        feature_frame: pd.DataFrame,
        anchor_date: pd.Timestamp,
        target_site: str,
    ) -> Optional[tuple]:
        """Build pooled ALL-SITE training data for regression.

        Used by data-starved sites (``use_pooled_training=True``) to borrow
        signal from data-rich sites.  Keeps the ``site`` column for one-hot
        encoding so the model can learn site-specific intercepts (factor
        model analog).

        Returns ``(X_processed, y, transformer, X_raw_columns)`` or *None*
        if insufficient data.
        """
        anchor_date = pd.Timestamp(anchor_date)
        all_train = feature_frame[feature_frame["date"] <= anchor_date].copy()

        # Same interpolated-training logic as per-site
        if getattr(config, "USE_INTERPOLATED_TRAINING", True):
            all_train = all_train[
                all_train["da_raw"].notna() | all_train["da"].notna()
            ].copy()
            all_train["_target"] = all_train["da_raw"].fillna(all_train["da"])
        else:
            all_train = all_train[all_train["da_raw"].notna()].copy()
            all_train["_target"] = all_train["da_raw"]

        min_samples = getattr(config, "POOLED_TRAINING_MIN_SAMPLES", 30)
        if len(all_train) < min_samples:
            return None

        all_train = add_temporal_features(all_train)
        y = all_train["_target"].astype(float).copy()

        # One-hot encode 'site' manually so create_transformer (which
        # filters to numeric columns) preserves site identity as dummy
        # columns — this gives the model site-specific intercepts.
        # Cast to int so select_dtypes(include=[np.number]) keeps them.
        site_dummies = pd.get_dummies(all_train["site"], prefix="site").astype(int)
        all_train = pd.concat([all_train.drop(columns=["site"]), site_dummies], axis=1)
        ...
        # Leakage assertion on pooled data
        assert all_train["date"].max() <= anchor_date, (
            f"TEMPORAL LEAK in pooled data: max date "
            f"{all_train['date'].max().date()} > anchor {anchor_date.date()}"
        )

        transformer, X_raw = create_transformer(all_train, drop_cols)
        X_processed = transformer.fit_transform(X_raw)

        return X_processed, y, transformer, X_raw.columns.tolist()
    ...
    def generate_single_forecast(
        ...
        # Quantile regression: predict a higher quantile for spike-sensitive forecasting
        q_alpha = get_site_quantile_alpha(site)
        if q_alpha is not None:
            xgb_params["quantile_alpha"] = q_alpha
