...
All five Washington sites fall in the \textbf{high-skill} tier on the paper sample ($R^2$ range 0.286--0.787, all upper-bound CIs $\geq 0.535$); Copalis leads at $R^2 = 0.787$ [0.700, 0.842], and Figure~\ref{fig:pred-vs-obs} illustrates how the ensemble tracks observed DA dynamics at this site over the full 21-year record. The Quinault and Long Beach point estimates ($R^2 = 0.286$ and $0.410$) sit at the lower end of this band on the single-seed paper sample, but their bootstrap CIs overlap the WA cluster and the 5-seed bootstrap on the post-2022 holdout (Table~\ref{tab:temporal-holdout}) confirms both remain in the $+0.55$--$+0.58$ band on the unbiased window. Clatsop Beach ($R^2 = 0.260$), Coos Bay ($R^2 = 0.150$), and Gold Beach ($R^2 = 0.040$) form the \textbf{moderate-skill} tier. Cannon Beach ($R^2 = -0.044$) and Newport ($R^2 = -0.081$) form the \textbf{low-skill} tier. Cannon Beach achieves 92.2\% classification accuracy only because nearly all readings are Low; Newport's wide confidence interval [$-0.651$, $+0.061$] reflects episodic, unpredictable bloom events (Table~\ref{tab:predictability-ceiling}).
...
The difference between development-set $R^2$ (0.414, seed = 42) and held-out test-set $R^2$ (0.241, seed = 123) merits interpretation. First, the gap is expected by design: per-site configurations were tuned on seed = 42, so reporting on a separate seed produces less biased estimates. Second, the held-out set's larger sampling fraction (40\% vs.\ 20\%) includes more Oregon test points, and the five Oregon sites---three of which have negative $R^2$---disproportionately depress the aggregate. Washington sites achieve $R^2 = 0.286$--$0.787$ on the paper sample and $0.42$--$0.58$ on the 5-seed post-2022 holdout, confirming that tuned configurations generalize where sufficient data quality exists; the aggregate $R^2$ reflects Oregon data sparsity, not overfitting. The 5-seed bootstrap on the post-2022 holdout (Table~\ref{tab:temporal-holdout}) further confirms this: across seeds 42--46, pooled holdout $R^2$ ranges 0.32--0.55 with mean $0.433 \pm 0.092$, showing that single-seed differences of this magnitude are dominated by sampling variation at $N \approx 160$, not by tuning leak.
...
DATect provides the first quantitative one-week-ahead domoic acid forecasting system for Washington and Oregon. Under leak-free retrospective evaluation on a held-out test set, all five Washington sites achieve high skill ($R^2 = 0.286$--$0.787$ on the paper sample), while Oregon performance is limited by sparser and less regular monitoring. The resulting scientific message is not simply that DATect can forecast DA, but that regional monitoring quality currently places the strongest upper bound on forecast skill.
