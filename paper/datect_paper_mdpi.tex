...
\midrule
\textbf{Per-site sel.}$^{\dagger}$ & \textbf{0.241} [.125, .364] & \textbf{6.51} [5.89, 7.22] & \textbf{17.50} & --- \\
XGBoost          & 0.276 [.185, .379] & 6.58 [5.90, 7.25] & 17.08 & $-$.035 [$-$.052, $-$.018] \\
Random Forest    & 0.195 [.060, .324] & 6.46 [5.79, 7.23] & 18.02 & $+$.046 [$+$.023, $+$.069] \\
Linear (Ridge)   & 0.203 [.072, .326] & 6.61 [5.93, 7.39] & 17.93 & $+$.038 [$+$.001, $+$.071] \\
Na\"ive Persist. & $-$.426 [$-$.951, $-$.034] & 7.73 [6.84, 8.76] & 23.98 & $+$.667 [$+$.310, $+$1.16] \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
...
XGBoost achieves the highest individual model $R^2 = 0.276$, modestly above the ensemble ($R^2 = 0.241$). That the ensemble slightly underperforms XGB-everywhere reflects that per-site RF selection at several WA sites trades a small amount of pooled $R^2$ for better per-site fit; this is a consequence of pooled $R^2$ not being a weighted average of per-site values. Random Forest ($R^2 = 0.195$) and Linear/Ridge ($R^2 = 0.203$) trail XGB but remain competitive. Na\"ive persistence achieves $R^2 = -0.426$ at the pooled held-out scale, indicating that simply predicting the most recent DA observation is not sufficient once all sites are combined. This pooled result does not imply that persistence is useless at every site; rather, it underscores the importance of per-site modeling under strong regional heterogeneity. Paired bootstrap differences between the ensemble and individual ML models are small and statistically consistent with zero (Table~\ref{tab:model-comparison}), while the ensemble's advantage over na\"ive persistence is large and clearly significant ($\Delta R^2 = +0.667$\, $[+0.310,\, +1.16]$), reflecting the value of per-site model selection and feature engineering over raw autocorrelation.
...
\midrule
Copalis       & 277 & \textbf{.789} [.706, .843] & 2.82 [2.31, 3.39] & 5.28  & .827 & High \\
Long Beach    & 209 & \textbf{.629} [.551, .713] & 4.44 [3.47, 5.48] & 8.67  & .727 & High \\
Twin Harbors  & 225 & \textbf{.591} [.469, .759] & 4.67 [3.35, 6.21] & 12.25 & .809 & High \\
Quinault      & 181 & \textbf{.580} [.495, .733] & 4.16 [2.97, 5.62] & 10.18 & .762 & High \\
Kalaloch      & 235 & \textbf{.480} [.348, .641] & 3.77 [2.72, 5.08] & 10.14 & .809 & High \\
\addlinespace
Clatsop Bch   & 354 & .305 [$-$.183, .558] & 6.12 [4.83, 7.52] & 13.91 & .709 & Mod. \\
Gold Beach    & 257 & .079 [$-$.071, .231] & 8.24 [5.83, 11.09] & 23.04 & .770 & Mod. \\
\addlinespace
Cannon Bch    & 116 & $-$.044 [$-$.090, $-$.009] & 3.91 [1.40, 6.97] & 16.14 & .922 & Low \\
Coos Bay      & 109 & $-$.024 [$-$.654, .313] & 24.35 [18.55, 30.33] & 39.84 & .486 & Low \\
Newport       & 218 & $-$.135 [$-$.986, .056] & 11.07 [8.29, 14.79] & 27.03 & .541 & Low \\
\midrule
\textbf{Overall} & \textbf{2,181} & \textbf{.241} [.125, .364] & \textbf{6.51} [5.89, 7.22] & \textbf{17.50} & \textbf{.742} & --- \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
...
\begin{table}[H]
\caption{Per-site ensemble performance on the 2022--2024 temporal holdout, the most conservative unbiased evaluation. Multi-seed bootstrap with 5 random-anchor seeds (42--46) on the current configuration. Per-seed $N$ shown as mean across seeds; per-site $R^2$ shown as mean $\pm$ std and full single-seed range. The 2022--2024 window was selected specifically to be downstream of the latest per-site configuration adjustment (early 2019), so no tuning decision has ever inspected this period. Pooled holdout: $R^2 = 0.386 \pm 0.145$ (range 0.19--0.60), MAE $= 6.03 \pm 0.63$, $N \approx 160$ per seed.}
\label{tab:temporal-holdout}
\centering\scriptsize
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrccrl}
\toprule
\textbf{Site} & $\boldsymbol{N}$ & $\boldsymbol{R^2}$ \textbf{[95\% CI]} & \textbf{MAE [CI]} & \textbf{RMSE} & \textbf{Tier} \\
\midrule
Quinault      & 26 & \textbf{.564 $\pm$ .176} & [.348, .824]  & 2.95  & High \\
Long Beach    & 20 & \textbf{.549 $\pm$ .166} & [.303, .810]  & 3.88  & High \\
Twin Harbors  & 18 & \textbf{.533 $\pm$ .172} & [.373, .851]  & 4.08  & High \\
Copalis       & 29 & \textbf{.524 $\pm$ .156} & [.290, .710]  & 3.41  & High \\
Clatsop Bch   & 12 & \textbf{.495 $\pm$ .201} & [.299, .881]  & 5.99  & High \\
Kalaloch      &  9 & \textbf{.420 $\pm$ .169} & [.208, .677]  & 2.80  & Mod. \\
\addlinespace
Coos Bay      & 10 & \textbf{.697 $\pm$ .089} & [.556, .818]  & 11.82 & High$^\dagger$ \\
\addlinespace
Newport       & 20 & $-$1.06 $\pm$ 1.70 & [$-$4.40, .375]  & 16.87 & Low$^\ddagger$ \\
Gold Beach    & 17 & $-$2.14 $\pm$ 3.21 & [$-$8.46, $-$.082] & 5.83 & Low$^\ddagger$ \\
Cannon Bch    & --- & no spikes in holdout & --- & --- & N/A$^*$ \\
\midrule
\textbf{Overall} & \textbf{$\sim$160} & \textbf{.386 $\pm$ .145} & \textbf{[.188, .599]} & \textbf{6.03} & --- \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
\footnotesize{$N$: mean per-seed test points across 5 seeds. R$^2$ column: 5-seed mean $\pm$ std and bracketed range over single-seed values; this dispersion reflects measurement-set noise at small $N$, not tuning noise. MAE in \si{\microgrampeg}. $^\dagger$Coos Bay $R^2$ is high but unreliable: only $\approx 10$ holdout samples per seed; one extreme observation per seed drives both the apparent skill and the MAE. $^\ddagger$Newport and Gold Beach: catastrophic negative $R^2$ reflects very small $N$ amplifying any miscalibration; these sites have insufficient holdout data for a stable measurement. $^*$Cannon Beach 2022--2024 data is sparse with no spike events ($>$\SI{20}{\microgrampeg}); not evaluable as a regression target in this window.}
\end{table}

\begin{table}[H]
\caption{Persistence-forecast $R^2$ ceiling per site, computed as the squared lag-1 autocorrelation ($\rho^2$) of consecutive raw DA measurements. The ceiling bounds the maximum $R^2$ achievable by a \emph{persistence-only} (last-value) one-step-ahead forecast; models that incorporate environmental predictors (SST, BEUTI, PDO, discharge, etc.) can and do exceed $\rho^2$, as illustrated by Gold Beach ($\rho^2 = 0.002$, held-out $R^2 = 0.079$). Sites with near-zero ceilings indicate that persistence alone carries very little information about future DA levels.}
...
Long Beach    & 698 & 0.881 & 0.776 & 0.629 \\
Twin Harbors  & 690 & 0.863 & 0.744 & 0.591 \\
Quinault      & 564 & 0.836 & 0.698 & 0.580 \\
\addlinespace
Clatsop Beach & 1087 & 0.501 & 0.251 & 0.305 \\
Cannon Beach  &  305 & 0.443 & 0.197 & $-$0.044 \\
Newport       &  708 & 0.191 & 0.036 & $-$0.135 \\
Coos Bay      &  335 & 0.054 & 0.003 & $-$0.024 \\
Gold Beach    &  717 & 0.038 & 0.002 & 0.079 \\
\bottomrule
\end{tabular*}
\vspace{0.3em}\\
...
All five Washington sites fall in the \textbf{high-skill} tier ($R^2 = 0.480$--$0.789$), confirming that strong performance is not an artifact of per-site tuning; Copalis leads at $R^2 = 0.789$ [0.706, 0.843], and Figure~\ref{fig:pred-vs-obs} illustrates how the ensemble tracks observed DA dynamics at this site over the full 21-year record. Two Oregon sites show \textbf{moderate} skill: Clatsop Beach ($R^2 = 0.305$) and Gold Beach ($R^2 = 0.079$) are marginally positive. Cannon Beach, Coos Bay, and Newport form the \textbf{low-skill} tier ($R^2 \leq 0$). Cannon Beach ($R^2 = -0.044$) achieves 92.2\% classification accuracy only because nearly all readings are Low; Newport's wide confidence interval [$-0.986$, $+0.056$] reflects episodic, unpredictable bloom events (Table~\ref{tab:predictability-ceiling}).

\subsection{Temporal Holdout and Multi-Seed Sensitivity}
\label{sec:results-temporal-holdout}

The 2022--2024 temporal holdout (Table~\ref{tab:temporal-holdout}) provides the strictest unbiased validation: it is downstream of the latest per-site configuration adjustment, and the 5-seed bootstrap directly quantifies how much single-seed point estimates can vary at this $N$. Pooled holdout $R^2 = 0.386 \pm 0.145$ (single-seed range 0.19--0.60 across seeds 42--46). The standard-eval value of 0.241 (Table~\ref{tab:model-comparison}) and this holdout value are statistically indistinguishable once seed dispersion is accounted for, confirming that the model does not specifically benefit from the development-set sample.

Five Washington sites form a stable high-skill cluster with 5-seed mean $R^2$ between 0.42 and 0.56 and inter-seed std $\approx 0.17$. Oregon sites split: Clatsop Beach achieves comparable skill to WA (0.50 $\pm$ 0.20), Coos Bay shows high apparent $R^2$ that is unreliable due to small $N \approx 10$, and Newport and Gold Beach exhibit catastrophic negative $R^2$ driven by small holdout samples amplifying any prediction miscalibration. Cannon Beach has insufficient 2022--2024 data with no spike events (\textgreater\SI{20}{\microgrampeg}) to evaluate as a regression target.

A separate leak-test, in which we replaced the hand-tuned per-site values in \texttt{per\_site\_models.py} with the winners of a constrained 3-dimensional grid search (ensemble weights $\times$ prediction clip quantile $\times$ prediction clip ceiling) tuned exclusively on pre-2022 data via 3-fold chronological CV, confirmed that the hand-tuned configuration is not meaningfully overfitting the 2022--2024 holdout: the leak-free grid winners scored $R^2 = 0.434 \pm 0.124$ on the same 5-seed bootstrap (Δ vs.\ hand-tuned $= +0.05 \pm 0.07$, within seed noise). We did detect a small leak on the 2019--2022 validation window (hand-tuned 0.377 vs.\ grid 0.315, Δ $= -0.06 \pm 0.07$), confirming that the hand-tuning used implicit information from years overlapping the validation window. Crucially, this leak did not propagate to the post-2022 holdout, so the headline holdout numbers reported above remain a valid unbiased estimate of forecast skill.
...
The difference between development-set $R^2$ (0.414, seed = 42) and held-out test-set $R^2$ (0.241, seed = 123) merits interpretation. First, the gap is expected by design: per-site configurations were tuned on seed = 42, so reporting on a separate seed produces less biased estimates. Second, the held-out set's larger sampling fraction (40\% vs.\ 20\%) includes more Oregon test points, and the five Oregon sites---three of which have negative $R^2$---disproportionately depress the aggregate. Washington sites achieve $R^2 = 0.480$--$0.789$ on both seeds, confirming that tuned configurations generalize where sufficient data quality exists; the aggregate $R^2$ reflects Oregon data sparsity, not overfitting. The 5-seed bootstrap on the post-2022 holdout (Table~\ref{tab:temporal-holdout}) further confirms this: across seeds 42--46, pooled holdout $R^2$ ranges 0.19--0.60 with mean $0.386 \pm 0.145$, showing that single-seed differences of this magnitude are dominated by sampling variation at $N \approx 160$, not by tuning leak.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Manual per-site tuning}: Feature subsets, clipping rules, and model choices were selected on a development split. A perturbation study (Appendix~\ref{app:perturbation-sensitivity}) confirms that RF hyperparameters, feature subsets, and monotonic constraints are insensitive to the specific values chosen, while clipping thresholds and per-site model selection are well-calibrated. The held-out seed-123 evaluation and five-seed stability analysis reduce optimistic bias, though a fully nested temporal tuning workflow would provide a still stronger generalization estimate. Because the development split samples all years, post-2019 patterns can still influence design choices indirectly (even when final metrics emphasize a separate held-out seed); temporal holdout and stability analyses mitigate but do not remove that possibility. To quantify the residual effect, we ran an explicit leak-test comparing the hand-tuned configuration against a constrained 3-dimensional grid search (ensemble weights $\times$ prediction clip quantile $\times$ prediction clip ceiling) tuned strictly on pre-2022 data via chronological 3-fold CV. The leak-free grid winners scored 0.06 R$^2$ lower than hand-tuned on the 2019--2022 validation window (confirming a small validation-window leak from the seed-42 development sampling) but 0.05 R$^2$ higher on the untouched 2022--2024 holdout (within the 0.07 seed-noise band), demonstrating that the leak did not propagate to the post-2022 unbiased estimate. The headline holdout numbers (Section~\ref{sec:results-temporal-holdout}) remain a valid generalization estimate. A full 18-dimensional Optuna search using the same proper protocol produced gains on validation (+0.07 R$^2$) that fully reversed on the holdout ($-0.16$ R$^2$), confirming the model is feature-limited rather than hyperparameter-limited under broader search.
...
Per-site dev-set selection (this paper) & 0.241 & 6.51 & 17.50 \\
