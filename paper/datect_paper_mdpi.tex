...
Cannon Beach  &  13 & 0.000$^\ddagger$ & 0.03 & 0.03  & Moderate \\
...
\textbf{Moderate sites} (3 of 10): Clatsop Beach ($R^2 = 0.292$) and Gold Beach ($R^2 = 0.051$) show marginally positive skill, while Cannon Beach ($R^2 = -0.045$) is near zero. Clatsop Beach benefits from interpolated training, which provides gap-filled data that captures seasonal DA patterns between sparse raw measurements. Cannon Beach, despite having a mean DA of just \SI{1.06}{\microgrampeg}, achieves 92.2\% classification accuracy due to the dominance of the Low category.

\textbf{Low-skill sites} (2 of 10, $R^2 < 0$): Coos Bay ($R^2 = -0.038$) and Newport ($R^2 = -0.243$) have negative $R^2$, indicating the ensemble is worse than predicting the site mean. Coos Bay is notable for having the highest mean DA and the largest MAE (\SI{24.55}{\microgrampeg}), reflecting extreme variance in DA concentrations at this site. Newport's wide confidence interval [$-1.512$, $-0.017$] reflects high variability driven by episodic, unpredictable bloom events in its distinct oceanographic regime.
...
A targeted perturbation study (Appendix~\ref{app:perturbation-sensitivity}) further validates these design choices. Random Forest hyperparameters are genuinely insensitive across four configurations spanning depth 6--16 and 200--800 estimators ($|\Delta R^2| < 0.001$). Per-site prediction clipping is well-calibrated: relaxing all quantile thresholds to 0.99 costs $\Delta R^2 = -0.030$, confirming that the current site-specific quantiles (0.95--0.98) provide meaningful regularization. Multi-seed evaluation across five random seeds yields stable Washington performance (mean $R^2 = 0.61 \pm 0.05$) but near-random Oregon results (mean $R^2 = -0.03 \pm 0.25$), reinforcing that the Oregon skill deficit reflects data scarcity rather than tuning fragility.
...
Two additional diagnostic analyses characterize where and how the ensemble fails. First, an episodic-error decomposition shows that 88.6\% of total squared error is concentrated in the top 5\% of worst-performing test weeks ($n = 110$ rows). The five largest individual errors are all Oregon observations exceeding \SI{100}{\microgrampeg}, consistent with the near-zero persistence-forecast ceilings at Newport, Coos Bay, and Gold Beach (Table~\ref{tab:predictability-ceiling}): persistence alone carries almost no signal for these extreme events, and they contribute a disproportionate share of aggregate MSE. Environmental predictors provide additional structure at these sites (Gold Beach $R^2 = 0.051 > \rho^2 = 0.002$), but extreme bloom magnitudes remain the primary driver of error. Second, a bias analysis in the 12--30~\si{\microgrampeg} transition zone ($n = 311$ observations) reveals a systematic under-prediction of $+2.2$~\si{\microgrampeg} (i.e., predictions are on average 2.2~\si{\microgrampeg} below actuals). This conservative bias near bloom thresholds is a predictable consequence of MSE-optimized regression on a right-skewed target and motivates the hybrid alert's use of the transition-focused classifier to compensate.
...
\begin{table}[H]
\caption{Per-category classification performance under leak-free validation ($n = 2{,}181$). The Low category achieves the highest reliability, while High and Extreme categories are limited by sample size. Overall accuracy 75.1\% (macro-average precision 0.605, recall 0.581, F1 0.591).}
...
\begin{table}[H]
\caption{Perturbation sensitivity analysis. Each row modifies one design choice relative to the baseline configuration. $\Delta R^2$ is relative to baseline ($R^2 = 0.409$).}
