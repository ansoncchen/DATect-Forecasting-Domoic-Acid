...
\footnotesize
\begin{tabularx}{\fulllength}{lllllllX}
\toprule
\textbf{System} & \textbf{Region} & \textbf{Sites} & \textbf{Period} & \textbf{Method} & \textbf{Target} & \textbf{Horizon} & \textbf{Depl.} \\
\midrule
PNW HAB Bulletin \cite{McCabe2023} & WA/OR & $\sim$20 & 2017-- & Qual.\ expert & DA risk (categ.) & 1--2 wk & Yes \\
\addlinespace
Maine PST \cite{Grasso2019} & Maine & $\sim$10 & $\sim$5 yr & CNN & PST (not DA) & 1 wk & No \\
\addlinespace
Galician RF \cite{Alaez2021} & NW Spain & $\sim$5 & $\sim$10 yr & RF/AdaBoost & PN bloom (bin.) & --- & No \\
\addlinespace
C-HARM \cite{Anderson2016} & California & Grid & Ongoing & Sat.+model & PN prob. & Nowcast & Yes \\
\addlinespace
Monterey Bay LR \cite{Lane2009} & California & 1 & $\sim$8 yr & Logistic reg. & PN bloom (bin.) & --- & No \\
\addlinespace
\textbf{DATect (this study)} & \textbf{WA/OR} & \textbf{10} & \textbf{21 yr} & \textbf{XGB+RF} & \textbf{DA conc.+categ.} & \textbf{1 wk} & \textbf{Yes} \\
\bottomrule
\end{tabularx}
\end{adjustwidth}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{p{2.5cm}p{3cm}llX}
\toprule
...
Forecast uncertainty bands are produced via quantile regression \cite{Koenker1978} (XGBoost \texttt{reg:quantile} at the 5th/50th/95th percentiles for real-time forecasts; 10th/50th/90th for retrospective summaries), yielding an empirical predictive band rather than a fully calibrated posterior interval. Bootstrap 95\% CIs ($B = 10{,}000$; percentile method) reported for retrospective metrics summarize variation across held-out prediction rows.
...
\toprule
\textbf{Model} & $\boldsymbol{R^2}$ \textbf{[95\% CI]} & \textbf{MAE [95\% CI] (\si{\microgrampeg})} & \textbf{RMSE} & \textbf{$\Delta R^2$ vs.\ ens.\ [95\% CI]} \\
\midrule
\textbf{Per-site sel.\ (XGB/RF)}$^{\dagger}$ & \textbf{0.215} [0.096, 0.342] & \textbf{6.42} [5.78, 7.15] & \textbf{17.79} & --- \\
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{$^\dagger$ All per-site ensemble weights are $w \in \{0,1\}$ in the current configuration, so ``Per-site selection'' acts as a site-wise model selector (XGBoost at 4 sites, Random Forest at 6 sites) rather than a blended average; the ensemble formulation is retained for generality (Section~\ref{sec:model-selection}). $\Delta R^2$ is computed as $R^2_{\text{comparator}} - R^2_{\text{ensemble}}$: positive values indicate the ensemble outperforms the comparator; negative values indicate the comparator outperforms the ensemble.}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{lrrllrl}
\toprule
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{Adjusted $R^2$ values (not shown) differ from $R^2$ by $< 0.02$ at all sites; the modest sample-size-to-feature ratios at data-sparse sites are mitigated by per-site feature subsetting (Appendix~\ref{app:features}).}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{lrllrl}
\toprule
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{Bootstrap 95\% CIs ($B = 10{,}000$; percentile method). $^\dagger$Clatsop Beach and Coos Bay shift from Moderate/Low-skill on the standard eval to High-skill on the temporal holdout ($R^2 > 0.4$); Coos Bay's large improvement ($R^2 = 0.616$ vs.\ $-0.038$) is difficult to interpret given the small post-2019 sample ($N = 89$). $^\ddagger$Cannon Beach ($N = 13$) has insufficient post-2019 measurements for stable per-site estimates; essentially all 13 holdout observations are near-zero DA, making this a degenerate sample rather than genuine forecast skill. Bootstrap CI omitted due to insufficient sample size.}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{lccccl}
\toprule
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{Clip~$q$: predictions clipped to this quantile of training targets. ``---'' = global default ($q = 0.99$) or no hard ceiling. Clip max in \si{\microgrampeg}. All sites were assigned single-model weights ($w \in \{0,1\}$); blended weights were tested but did not improve over single-model selection (Appendix~\ref{app:weight-robustness}).}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{lcccX}
\toprule
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{mcw = min\_child\_weight; ss = subsample; cs = colsample\_bytree. Clatsop Beach uses all global defaults. All other sites override every listed parameter.}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{Xcccc}
\toprule
...
All derived env.\ features (4 features)$^\ddagger$ & 0.455 & $-0.002$ & 6.18 & $+0.03$ \\
...
\end{adjustwidth}
\vspace{0.3em}
\noindent\footnotesize{$^\dagger$Includes: K490, K490$^2$, fluorescence efficiency, Chl-$a$ anomaly, PDO--ONI phase, BEUTI relaxation, PN threshold, and Chl-$a$. Individual feature-level removals (not shown) all fall within $|\Delta R^2| \leq 0.005$. $^\ddagger$Count reflects the 4 derived features active in this configuration checkpoint; the full set of 7 derived features explored across development is documented in Appendix~\ref{app:feature-engineering}.}
\end{table}
...
\begin{adjustwidth}{-\extralength}{}
\footnotesize
\begin{tabularx}{\fulllength}{Xrrr}
\toprule
