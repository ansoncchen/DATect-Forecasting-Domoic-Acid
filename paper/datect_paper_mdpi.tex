...
% Standard eval (seed=123, 40%, N=2,181): Ensemble R²=0.215, MAE=6.42. CIs pending eval_paper_metrics.py re-run.
...
% NOTE: Table 1/2 CIs use placeholder values from standard eval scoreboard pending full bootstrap re-run on Hyak.
%
...
As an additional validation entirely free of tuning contact, we report a \textit{temporal holdout} evaluation in which test points are restricted to 2019--2023 ($N = 1{,}036$, Table~\ref{tab:temporal-holdout}). All per-site configurations and hyperparameter grids were developed exclusively against pre-2019 data; the post-2019 period is entirely unseen by any configuration decision. This provides an unbiased estimate of generalization to future periods, independent of random-seed selection. Training for each test point in the holdout still uses all data before the anchor date, so the hold-out designation affects only which dates appear as test points, not the expanding-window training mechanism.
...
\midrule
\textbf{Ensemble (XGB+RF)} & \textbf{0.215} [0.089, 0.344] & \textbf{6.42} [5.79, 7.15] & \textbf{17.72} & --- \\
...
Cannon Beach   & 116 & $-$0.045 [$-$0.091, $-$0.010] & 3.93 [1.38, 6.97] & 16.15 & 0.922 & Moderate \\
...
Newport        & 218 & $-$0.243 [$-$1.871, $-$0.038] & 10.30 [7.48, 14.38] & 28.64 & 0.615 & Low-skill \\
\midrule
\textbf{Overall} & \textbf{2,181} & \textbf{0.215} [0.089, 0.344] & \textbf{6.42} [5.79, 7.15] & \textbf{17.72} & \textbf{0.741} & --- \\
\bottomrule
\end{tabular}
\vspace{0.3em}\\
\footnotesize{Adjusted $R^2$ values (not shown) differ from $R^2$ by $< 0.02$ at all sites due to favorable sample-size-to-feature ratios ($N/p > 3$ at all sites). Bootstrap 95\% CIs to be updated from \texttt{eval\_paper\_metrics.py} re-run on Hyak.}
\end{table}

\begin{table}[H]
\caption{Per-site ensemble performance on the 2019--2023 temporal holdout ($N = 1{,}036$). All per-site configurations and hyperparameter grids were developed exclusively on pre-2019 data; no tuning decisions were informed by any post-2018 date. Training for each test point still uses all data before the anchor date. The temporal holdout therefore provides an unbiased estimate of generalization to future periods. Pooled $R^2 = 0.315$, WA mean $R^2 = 0.549$, OR mean $R^2 = 0.113$.}
\label{tab:temporal-holdout}
\small
\begin{tabular}{lrrrll}
\toprule
\textbf{Site} & $\boldsymbol{N}$ & $\boldsymbol{R^2}$ & \textbf{MAE (\si{\microgrampeg})} & \textbf{RMSE (\si{\microgrampeg})} & \textbf{Tier} \\
\midrule
Quinault      & 144 & \textbf{0.735} & 2.72 & 8.95  & High-skill \\
Copalis       & 152 & \textbf{0.629} & 3.20 & 10.22 & High-skill \\
Long Beach    & 101 & \textbf{0.628} & 5.27 & 9.11  & High-skill \\
Twin Harbors  & 103 & \textbf{0.461} & 5.18 & 14.15 & High-skill \\
Clatsop Beach & 118 & \textbf{0.522} & 9.78 & 18.34 & Moderate \\
Coos Bay      &  89 & \textbf{0.616} & 8.08 & 22.87 & Moderate \\
\addlinespace
Kalaloch      &  82 & 0.289 & 2.55 & 7.42  & Moderate \\
Cannon Beach  &  13 & 0.000 & 0.03 & 0.04  & Moderate \\
Newport       & 124 & $-$0.305 & 12.88 & 30.44 & Low-skill \\
Gold Beach    & 110 & $-$0.271 & 10.56 & 29.14 & Low-skill \\
\midrule
\textbf{Overall} & \textbf{1,036} & \textbf{0.315} & \textbf{6.55} & \textbf{17.91} & --- \\
\bottomrule
\end{tabular}
\vspace{0.3em}\\
\footnotesize{Coos Bay shows markedly higher skill on the temporal holdout ($R^2 = 0.616$) than on the standard random-split evaluation ($R^2 = -0.038$), suggesting a possible post-2019 regime shift in DA dynamics at this site consistent with the anomalous marine heatwave conditions of 2019--2021. Cannon Beach ($N = 13$) has insufficient post-2019 measurements for stable per-site estimates.}
\end{table}

\begin{table}[H]
\caption{Theoretical $R^2$ ceiling per site, computed as the squared lag-1 autocorrelation ($\rho^2$) of consecutive raw DA measurements. The ceiling represents the maximum $R^2$ achievable by any optimal linear one-step-ahead forecast given the observed autocorrelation structure. Sites with near-zero ceilings are fundamentally unpredictable at the weekly horizon regardless of model architecture.}
...
Hybrid alert (classifier + \SI{12}{\microgrampeg} trigger) & \textbf{0.734} & \textbf{0.859} & 0.344 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Temporal Holdout Validation and Diagnostic Findings}
\label{sec:results-temporal-holdout}

Table~\ref{tab:temporal-holdout} reports performance on the 2019--2023 temporal holdout ($n = 1{,}036$), a set of test points entirely unseen by any tuning decision. The pooled held-out $R^2 = 0.315$ exceeds the standard seed-123 result (0.215), consistent with the post-2019 period containing proportionally more Washington test points and exhibiting stronger DA autocorrelation structure in that region. Washington sites achieve a mean $R^2 = 0.549$, while Oregon sites average $R^2 = 0.113$. The one notable anomaly is Coos Bay ($R^2 = 0.616$ on holdout vs.\ $-0.038$ on the standard eval), which may reflect a post-2019 oceanographic regime shift driven by the 2019--2021 marine heatwave and increased monitoring regularity during the post-heatwave recovery period; this site warrants targeted follow-up.

Two additional diagnostic analyses characterize where and how the ensemble fails. First, an episodic-error decomposition shows that 88.6\% of total squared error is concentrated in the top 5\% of worst-performing test weeks ($n = 110$ rows). The five largest individual errors are all Oregon observations exceeding \SI{100}{\microgrampeg}, consistent with the near-zero autocorrelation ceilings at Newport, Coos Bay, and Gold Beach (Table~\ref{tab:predictability-ceiling}): these extreme events are essentially unpredictable by any lag-based forecaster, and they contribute a disproportionate share of aggregate MSE. Second, a bias analysis in the 12--30~\si{\microgrampeg} transition zone ($n = 311$ observations) reveals a systematic under-prediction of $+2.2$~\si{\microgrampeg} (i.e., predictions are on average 2.2~\si{\microgrampeg} below actuals). This conservative bias near bloom thresholds is a predictable consequence of MSE-optimized regression on a right-skewed target and motivates the hybrid alert's use of the transition-focused classifier to compensate.
...
\begin{enumerate}[nosep]
    \item \textbf{Low-skill Oregon sites}: Two of 10 sites (Coos Bay, Newport) have negative $R^2$, and three additional Oregon sites have near-zero skill. Lag-1 autocorrelation analysis (Table~\ref{tab:predictability-ceiling}) reveals theoretical $R^2$ ceilings of 0.003--0.036 for Newport, Coos Bay, and Gold Beach, indicating that near-zero predictive skill at these sites reflects fundamental temporal structure in the data rather than model limitations alone. Even a perfect one-step-ahead predictor would achieve $R^2 < 0.04$ at these sites given the observed autocorrelation structure. The paper cannot fully separate data sparsity from true regional oceanographic complexity, but the autocorrelation ceilings establish that architecture improvements alone cannot close the performance gap.
...
Per-site dev-set selection (this paper) & 0.215 & 6.42 & 17.72 \\
\bottomrule
\end{tabular}
