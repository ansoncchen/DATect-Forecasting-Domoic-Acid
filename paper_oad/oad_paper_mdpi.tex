...
\caption{OAD 3D convolutional masked-autoencoder architecture. Input is a 4-channel (chl-a, Kd490, nFLH, SST) 4-day rolling 8-day-composite cube. The encoder applies four 3D conv + stride-2 downsampling blocks to a latent of dimension 32; the decoder symmetrically reconstructs the input. During training, 50\% of input pixels are randomly masked (mae050 variant) and the reconstruction loss is computed only on ocean pixels in the static mask, with cloud-flagged (NaN) pixels excluded per-pixel.}
