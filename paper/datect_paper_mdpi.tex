...
Current DA risk management in the PNW is reactive: beaches are closed only after measured toxin levels exceed the FDA regulatory limit of \SI{20}{\microgrampeg} \cite{McCabe2023}, giving managers no advance warning and leaving commercial fisheries with no time to adapt harvest plans. The primary forecasting tool is the NOAA Pacific Northwest HAB Bulletin \cite{McCabe2023}, issued biweekly to monthly during peak bloom season. The Bulletin synthesizes environmental, biological, and oceanographic data into qualitative risk categories (low/medium/high) that provide strategic planning value \cite{Jin2024} but do not produce numerical DA concentration predictions and operate at seasonal rather than weekly resolution. Complementary regional resources---Oregon's phytoplankton monitoring program \cite{McKibben2015}, the SoundToxins citizen-science network \cite{Trainer2023}, and the LiveOcean circulation model \cite{MacCready2021}---extend observational coverage but do not forecast DA. The California C-HARM system \cite{Anderson2016} provides bloom-probability nowcasts for California but does not extend to Washington or Oregon (Table~\ref{tab:system-comparison}).
...
All five Washington sites fall in the \textbf{high-skill} tier ($R^2 = 0.480$--$0.789$), confirming that strong performance is not an artifact of per-site tuning; Copalis leads at $R^2 = 0.789$ [0.710, 0.844]. Three Oregon sites show \textbf{moderate} skill: Clatsop Beach ($R^2 = 0.292$) and Gold Beach ($R^2 = 0.051$) are marginally positive, while Cannon Beach ($R^2 = -0.045$) is near zero but achieves 92.2\% classification accuracy due to near-universal Low DA readings. Coos Bay and Newport form the \textbf{low-skill} tier ($R^2 < 0$); Newport's wide confidence interval [$-1.512$, $-0.017$] reflects episodic, unpredictable bloom events at both sites (Table~\ref{tab:predictability-ceiling}).
...
Table~\ref{tab:temporal-holdout} reports performance on the 2019--2023 temporal holdout ($n = 1{,}036$). Pooled $R^2 = 0.315$ exceeds the standard result (0.215), as the post-2019 period contains proportionally more Washington test points and stronger autocorrelation. Washington sites achieve mean $R^2 = 0.549$; Oregon sites average $R^2 = 0.113$. The notable anomaly is Coos Bay ($R^2 = 0.616$ vs.\ $-0.038$ on the standard eval), possibly reflecting a post-2019 oceanographic regime shift during the 2019--2021 marine heatwave.
