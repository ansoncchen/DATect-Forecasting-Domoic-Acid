...
\begin{table}[H]
\caption{Per-site ensemble performance on the 2022--2023 temporal holdout, the most conservative unbiased evaluation. Multi-seed bootstrap with 5 random-anchor seeds (42--46) on the current configuration. Per-seed $N$ shown as mean across seeds; per-site $R^2$ shown as mean $\pm$ std and full single-seed range. The 2022--2023 window was selected specifically to be downstream of the latest per-site configuration adjustment (early 2019), so no tuning decision has ever inspected this period. Pooled holdout: $R^2 = 0.386 \pm 0.145$ (range 0.19--0.60), MAE $= 6.03 \pm 0.63$, $N \approx 160$ per seed.}
...
The 2022--2023 temporal holdout (Table~\ref{tab:temporal-holdout}) provides the strictest unbiased validation: it is downstream of the latest per-site configuration adjustment, and the 5-seed bootstrap directly quantifies how much single-seed point estimates can vary at this $N$. Pooled holdout $R^2 = 0.386 \pm 0.145$ (single-seed range 0.19--0.60 across seeds 42--46). The standard-eval value of 0.241 (Table~\ref{tab:model-comparison}) and this holdout value are statistically indistinguishable once seed dispersion is accounted for, confirming that the model does not specifically benefit from the development-set sample.
